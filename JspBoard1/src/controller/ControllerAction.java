  ...
  //Properties객체 생성
    ...
    FileInputStream f = null;//파일불러올때 
     ...
     while(keyiter.hasNext()){
       ...
       //요청클래스의 객체를 생성->newInstance(); =>객체생성 메서드
    ...
    private void requestPro(HttpServletRequest request,
    		                HttpServletResponse response) 
    throws ServletException, IOException {
    	String view=null;//요청명령어에 따라서 이동할 페이지를 저장->/list.jsp
    	//요청 명령어에 따른 요청명령어 클래스(=액션클래스)
    	/*
    	 * ListAction com=null; ListAction com=new ListAction();
    	 * WriteFormAction com=null; WriteFormAction com=new WriteFormAction();
    	 * ,,,
    	 */
    	//상속기법                                 //CommadAction com=new WriteFormAction();
    	CommandAction com=null;//CommadAction com=new ListAction();
    	//어떠한 자식클래스의 객체라도 부모형으로 형변환을 통해 받아올 수 있다.
    	try {
    		//1.요청명령어를 분리->jsp결정
    		String command=request.getRequestURI();//~프로젝트명~요청명령어까지
    		// /JspBoard1/list.do
    		System.out.println("request.getRequestURI()="+command);
    		// /JspBoard1
    		System.out.println("request.getContextPath()="+request.getContextPath());
    		if(command.indexOf(request.getContextPath())==0) {//일치되는 부분을 찾아라
    			command=command.substring(request.getContextPath().length());
    			System.out.println("실질적인 command="+command);// /list.do
    		}
    		//요청명령어->/list.do->action.ListAction객체를 ->requestPro()호출->/list.jsp
    		com=(CommandAction)commandMap.get(command);
    		System.out.println("com="+com);//action.ListAction@주소값
    		//요청명령어에 따라서 어떠한 페이지로 이동할지 결정
    		view=com.requestPro(request, response);//ListAction클래스를 작성
    		System.out.println("view="+view);// /list.jsp
    	}catch(Throwable e){
    		throw new ServletException(e);//서블릿 예외처리에 대한 에러발생
    	}//catch
    	//위에서 요청명령어에 해당하는 view로 데이터를 공유시키면서 이동
    	RequestDispatcher dispatcher=request.getRequestDispatcher(view);// /list.jsp
    	dispatcher.forward(request, response);//데이터 공유하면서 특정 jsp로 이동
    }
}
